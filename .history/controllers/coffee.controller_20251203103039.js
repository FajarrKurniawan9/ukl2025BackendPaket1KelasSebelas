...
export const postNewCoffeeMenu = async (req, res) => {
  ...
  if (name == null || price == null || quantity == null || size == null) {
    return res
      .status(400)
      .json({ message: "Name, Price, Quantity, and Size are required" });
  }
  ...
  if (!newCoffeeMenu) {
    return res.status(500).json({ message: "Failed to create new coffee menu" });
  }
  if (name === prisma.coffee.name) {
