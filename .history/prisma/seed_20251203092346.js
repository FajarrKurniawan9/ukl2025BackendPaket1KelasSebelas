...
async function main() {
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    // Only cashier can serve orders (get random cashier, not all users)
    const cashiers = users.filter((u) => u.role === "cashier");
    const randomCashier = cashiers[randomInRange(0, cashiers.length - 1)];
    ...
    for (let j = 0; j < numItems; j++) {
      ...
      await prisma.order_detail.create({
        data: {
          ...
          user_id: randomCashier.id, // Only cashier
          quantity: quantity,
    ...
    console.log(
      `   ✓ Created order #${order.id}: ${customerName} - ${orderType} (${numItems} items, served by ${randomCashier.name}) - Total: Rp ${totalPrice}`
    );
  }
